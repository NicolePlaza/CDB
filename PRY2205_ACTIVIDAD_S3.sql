...
INSERT INTO arriendo_propiedad VALUES (
    ...
    12467572,
    TO_DATE('14/10/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 15)),
    TO_DATE('20/03/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 7))
);
...
INSERT INTO arriendo_propiedad VALUES (
    ...
    10639521,
    TO_DATE('05/05/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 8)),
    NULL
);
...
INSERT INTO arriendo_propiedad VALUES (
    ...
    12817700,
    TO_DATE('02/04/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 9)),
    NULL
);
...
INSERT INTO arriendo_propiedad VALUES (
    ...
    9771046,
    TO_DATE('05/05/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 10)),
    NULL
);
...
INSERT INTO arriendo_propiedad VALUES (
    ...
    TO_DATE('02/03/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 8)),
    TO_DATE('30/12/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 6))
);
...
INSERT INTO arriendo_propiedad VALUES (
    ...
    TO_DATE('20/01/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 10)),
    TO_DATE('31/12/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 5))
);
...
INSERT INTO arriendo_propiedad VALUES (
    ...
    8533901,
    TO_DATE('13/01/'
            || to_char(EXTRACT(YEAR FROM sysdate) - 11)),
    NULL
);
...
SELECT
    ...
    || apmaterno_cli
    || ' '
    || nombre_cli                        AS "Nombre Completo Cliente",
    direccion_cli                        AS "Direccion Cliente",
    celular_cli                          AS "Celular Cliente",
    '$' || to_char(renta_cli, '999,999') AS "Renta Cliente",
    CASE
        WHEN renta_cli > 500000                  THEN
            'TRAMO 1'
        WHEN renta_cli BETWEEN 400000 AND 500000 THEN
            'TRAMO 2'
        WHEN renta_cli BETWEEN 200000 AND 399999 THEN
            'TRAMO 3'
        ELSE
            'TRAMO 4'
...
SELECT
    ...
    || to_char(round(AVG(e.sueldo_emp)),
               '9,999,999')    AS "SUELDO_PROMEDIO"
FROM
         empleado e
    INNER JOIN categoria_empleado c ON e.id_categoria_emp = c.id_categoria_emp
...
SELECT
    ...
    || to_char(round(AVG(p.valor_arriendo)),
               '9,999,999')         AS "PROMEDIO_ARRIENDO",
    round(AVG(p.superficie)) AS "PROMEDIO_SUPERFICIE",
    '$'
    || to_char(round(AVG(p.valor_arriendo) / AVG(p.superficie)),
               '99,999')            AS "VALOR_ARRIENDO_M2",
    CASE
        WHEN round(AVG(p.valor_arriendo) / AVG(p.superficie)) < 5000                 THEN
            'Económico'
        WHEN round(AVG(p.valor_arriendo) / AVG(p.superficie)) BETWEEN 5000 AND 10000 THEN
            'Medio'
        ELSE
            'Alto'
