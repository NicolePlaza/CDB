...
INSERT INTO TRABAJADOR VALUES (11846972,'5','OGAZ','VARAS','MARCO','OVALLE Nº5798 V/ OHIGGINS','27763209','31121959','01022017',253590,94,4,50,6,4);
INSERT INTO TRABAJADOR VALUES (14283083,'6','MONDACA','COLLAO','AUGUSTO','NUEVA COLON Nº1152','27357104','01011989','01092013',1144245,95,2,50,3,6);
...
FROM (
    SELECT
        ...
        TO_CHAR(NVL(tc.monto_ticket, 0), '$999,999,999') AS monto,
        
        TO_CHAR(
            CASE
                WHEN tc.monto_ticket IS NULL THEN 0
                WHEN tc.monto_ticket <= 50000 THEN 0
                WHEN tc.monto_ticket > 50000 AND tc.monto_ticket <= 100000 THEN tc.monto_ticket * 0.05
    ...
    FROM 
        ...
        LEFT JOIN ticket_con tc ON t.numrut = tc.numrut_t
        LEFT JOIN bono_ant ba ON 
            TRUNC(MONTHS_BETWEEN(SYSDATE, t.fecing) / 12) >= ba.limite_inferior
            AND TRUNC(MONTHS_BETWEEN(SYSDATE, t.fecing) / 12) <= ba.limite_superior
    WHERE
        i.porc_descto_isapre > 4
        AND TRUNC(MONTHS_BETWEEN(SYSDATE, NVL(t.fecnac, SYSDATE)) / 12) < 50
    ORDER BY
        NVL(tc.monto_ticket, 0) DESC,
        t.appaterno || ' ' || t.apmaterno || ' ' || t.nombre ASC
);
...
-- Caso 2: Vistas
CREATE SYNONYM trab_est FOR trabajador;
CREATE SYNONYM bon_esc FOR bono_escolar;

CREATE OR REPLACE VIEW v_aumentos_estudios AS
SELECT
    TO_CHAR(t.numrut) || '-' || t.dvrut AS rut,
    INITCAP(t.appaterno || ' ' || t.apmaterno || ' ' || t.nombre) AS trabajador,
    UPPER(be.descrip) AS bct_estudios,
    LPAD(be.porc_bono, 7, '0') AS porc_bono,
    TO_CHAR(t.sueldo_base, '$999,999,999') AS sueldo_actual,
    TO_CHAR(
        ROUND(t.sueldo_base * (be.porc_bono / 100))
    , '$999,999,999') AS aumento,
    TO_CHAR(
        t.sueldo_base + ROUND(t.sueldo_base * (be.porc_bono / 100))
    , '$999,999,999') AS sueldo_aumentado
FROM 
    trab_est t
    INNER JOIN bon_esc be ON t.id_escolaridad_t = be.id_escolar
...
SELECT COUNT(*) AS total_trabajadores FROM v_aumentos_estudios;
...
EXEC DBMS_STATS.GATHER_INDEX_STATS(ownname => 'PRY2205_S7', indname => 'IDX_TRABAJADOR_APM');
EXEC DBMS_STATS.GATHER_INDEX_STATS(ownname => 'PRY2205_S7', indname => 'IDX_TRABAJADOR_APM_2');