...
SELECT
    INITCAP(t.nombre) || ' ' || 
    INITCAP(t.appaterno) || ' ' ||
    INITCAP(t.apmaterno) AS "Nombre Completo Trabajador",
    
    TO_CHAR(t.numrut, 'FM99,999,999') || '-' || t.dvrut AS "RUT Trabajador",
    INITCAP(tt.desc_categoria) AS "Tipo Trabajador",
    
    INITCAP(c.nombre_ciudad) AS "Ciudad",
    
    TO_CHAR(t.sueldo_base, 'FM$999,999,999') AS "Sueldo Base"
    
FROM trabajador t
INNER JOIN comuna_ciudad c ON t.id_ciudad = c.id_ciudad
...
SELECT
    ...
    INITCAP (t.nombre) || ' ' ||
    INITCAP(t.appaterno) AS "Nombre Trabajador",    
    COUNT(tc.nro_ticket) AS "Total Tickets",
    TO_CHAR(SUM(tc.monto_ticket), 'FM$999,999,999') AS "Total Vendido",
    TO_CHAR(NVL(SUM(ct.valor_comision), 0), 'FM$999,999,999') AS "Comisión Total",
    INITCAP(tt.desc_categoria) AS "Tipo Trabajador",
    INITCAP(c.nombre_ciudad) AS "Ciudad"
...
SELECT
    ...
    INITCAP(t.nombre) || ' ' ||
    INITCAP(t.appaterno) AS "Trabajador Nombre",
    
    EXTRACT(YEAR FROM t.fecing) AS "Año Ingreso",
    TRUNC(MONTHS_BETWEEN(SYSDATE, t.fecing) / 12) AS "Años Antigüedad",

    CASE
        WHEN COUNT(af.numrut_carga) > 0 THEN 'Sí'
        ELSE 'No'
    END AS "Tiene Cargas",
    
    NVL(COUNT(af.numrut_carga), 0) AS "Cantidad Cargas",
    
    INITCAP(i.nombre_isapre) AS "Nombre Salud",
    
    TO_CHAR(t.sueldo_base, 'FM$999,999,999') AS "Sueldo Base",
    
    CASE
        WHEN UPPER(i.nombre_isapre) = 'FONASA' THEN
            TO_CHAR(t.sueldo_base * 0.01, 'FM$999,999,999')
        ELSE TO_CHAR(0, 'FM$999,999,999')
    END AS "Bono FONASA",
    
    CASE 
        WHEN TRUNC(MONTHS_BETWEEN(SYSDATE, t.fecing) / 12) <= 10 THEN
            TO_CHAR(t.sueldo_base * 0.10, 'FM$999,999,999')
        ELSE
            TO_CHAR(t.sueldo_base * 0.15, 'FM$999,999,999')
...
WHERE esc.fecter_estcivil IS NULL
    OR esc.fecter_estcivil > SYSDATE
